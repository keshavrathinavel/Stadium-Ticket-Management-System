      ...
      if self.entry.get()==""or self.entry2.get()==""or self.entry3.get()==""or self.entry4.get()=="":
        ...
        messagebox.showerror("Error","Password and Confirm Password Should Be Same",parent=self.root)
         ...
         try:
            stadium = mysql.connector.connect(host="localhost", user="root", password="lmaogg69", database="stadium")
            cur = stadium.cursor()
            cur.execute("select * from customer where email=%s",self.entry3.get())
            row=cur.fetchone()
            if row!=None:
               ...
               cur.execute("insert into customer values(%s,%s,%s,%s,%s)",(self.entry1.get(),self.entry2.get(),self.entry3.get(),self.entry4.get(),self.entry5.get()))
               stadium.commit()
               stadium.close()
